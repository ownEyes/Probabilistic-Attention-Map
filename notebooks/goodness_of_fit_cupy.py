...
def me(cal, obs):
    ...
    result = cp.mean(cal) - cp.mean(obs)
    return result
...
def rmse(cal, obs):
    ...
    """
    cal, obs = _check_inputs(cal, obs)
    result = cp.sqrt(cp.mean(cp.power(cal - obs, 2.0)))
    return result
...
def r_pearson(cal, obs):
    ...
    r_num = cp.add.reduce(xm * ym)
    r_den = cp.sqrt(cp.sum(xm * xm) * cp.sum(ym * ym))
    if r_den == 0.0:
...
def md(cal, obs, order=1):
    ...
    obs_mean = obs.mean()
    denominator = cp.sum(
        cp.power(cp.abs(cal - obs_mean) + cp.abs(obs - obs_mean), order)
    )
    if denominator == 0.0:
        warnings.warn("Index of agreement potential error is null! Return NaN.")
        return cp.nan

    nominator = cp.sum(cp.abs(cp.power(obs - cal, order)))
...
def rd(cal, obs):
    ...
    denominator = cp.sum(
        cp.power((cp.abs(cal - obs_mean) + cp.abs(obs - obs_mean)) / obs_mean, 2.0)
    )
    if denominator == 0.0:
        warnings.warn(
    ...
    nominator = cp.sum(cp.power((obs - cal) / obs, 2.0))
    return 1 - (nominator / denominator)
...
def rsd(cal, obs):
    ...
    """
    cal, obs = _check_inputs(cal, obs)
    denominator = cp.std(obs)
    if denominator > 0.0:
        return cp.std(cal) / denominator
    else:
...
def nse(cal, obs):
    ...
    cal, obs = _check_inputs(cal, obs)
    obs_mean = obs.mean()
    denominator = cp.sum(cp.power(obs - obs_mean, 2.0))
    if denominator == 0.0:
        warnings.warn("Observation variance is null. Return NaN.")
        return cp.nan
    return 1.0 - cp.sum(cp.power(obs - cal, 2.0)) / denominator
...
def mnse(cal, obs, order=1):
    ...
    cal, obs = _check_inputs(cal, obs)
    obs_mean = obs.mean()
    denominator = cp.sum(cp.abs(cp.power(obs - obs_mean, order)))
    if denominator == 0.0:
        warnings.warn("Observation variance is null. Return NaN.")
        return cp.nan
    return 1.0 - cp.sum(cp.abs(cp.power(obs - cal, order))) / denominator
...
def rnse(cal, obs):
    ...
    denominator = cp.sum(cp.power((obs - obs_mean) / obs_mean, 2.0))
    if denominator == 0.0:
        warnings.warn("Normalized variance of observation is null! Return NaN.")
        return cp.nan

    return 1.0 - cp.sum(cp.power((obs - cal) / obs, 2.0)) / denominator
...
def kge(cal, obs):
    ...
    std_ratio = cp.std(cal) / o_std
    mean_ratio = cp.mean(cal) / o_mean
...
def dg(cal, obs):
    ...
    den = cp.sum((obs - obs.mean()) ** 2.0)
    if den == 0.0:
        return cp.nan

    return 1.0 - cp.sum((obs - cal) ** 2.0) / den
...
def sdr(cal, obs):
    ...
    return cp.sqrt(cp.mean(cp.power((cal - obs) - cal.mean() + obs.mean(), 2.0)))
